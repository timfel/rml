...
(defun gud-gdb-marker-filter (string)
  ...
  (let ((output ""))
    ...
    (message "file:%s" (car gud-last-frame))
    (message "line:%s" (cdr gud-last-frame))
...
Obeying it means displaying in another window the specified file and line."
  ...
  (if gud-last-frame
	...
	(gud-display-line 
	 (car gud-last-frame) 
	 (car (cdr gud-last-frame))
	 (car (cdr (cdr gud-last-frame)))
	 (car (cdr (cdr (cdr gud-last-frame))))
	 (cdr (cdr (cdr (cdr gud-last-frame)))))
...
(defun gud-display-line (true-file sline scolumn eline ecolumn)
  (let* ((last-nonmenu-event t)	; Prevent use of dialog box for questions.
	...
	(progn
	  (save-excursion
	    ...
	    (save-restriction
	      (widen)
	      ;; adrpo: before we go to the next goal we should
	      ;; remove the highlight from the last one
	      (if hl-line-overlay (delete-overlay hl-line-overlay))
	      (goto-line sline)
	      (beginning-of-line nil)
	      ;; adrpo: make the highlight so we see what goal is about to be
	      ;; executed
	      (setq hl-line-overlay (make-overlay 1 1))
	      (overlay-put hl-line-overlay 'face hl-line-face)
	      (move-overlay hl-line-overlay (+ (point) scolumn) (+ (point) ecolumn) buffer)
	      (hl-line-highlight)
	      (setq pos (+ (point) scolumn))
	      (setq overlay-arrow-string "=>")
	      (or overlay-arrow-position
		  (setq overlay-arrow-position (make-marker)))
	      (set-marker overlay-arrow-position (+ (point) scolumn) (current-buffer)))
    ...
    (while (string-match gud-rmldb-marker-regexp
      ...
      (setq
       ...
       (cons (substring gud-rmldb-marker-acc (match-beginning 1) (match-end 1))
	     (cons (string-to-int (substring gud-rmldb-marker-acc
					     (match-beginning 2)
					     (match-end 2)))
		   (cons (string-to-int (substring gud-rmldb-marker-acc
						   (match-beginning 3)
...
(defvar rmldb-command-name "./run-program.sh"
  "RML executable debugger.")
