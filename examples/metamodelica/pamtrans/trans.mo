...
algorithm 
  ...
  matchcontinue (inExp)
    local
      ...
      Absyn.Exp e1,e2;
      Absyn.BinOp binop;
    case (Absyn.INT(integer = v)) then {Mcode.MLOAD(Mcode.N(v))};  "integer constant"
    case (Absyn.IDENT(ident = id)) then {Mcode.MLOAD(Mcode.I(id))};  "identifier id"
    case (Absyn.BINARY(exp1 = e1,binOp2 = binop,exp3 = e2)) "Arith binop: simple case, expr2 is just an identifier or constant expr1 binop expr2"
      equation 
        cod1 = transExpr(e1);
