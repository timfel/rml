...
static INLINE int rml_is_allocated_on_c_heap(void** p)
{
	...
	while (current != NULL)
	{
		if ( p > current->region && p < current->limit )
		{
			/* bingo, we have a hit */
			return 1;
		}
...
void rml_free_c_heap_region()
{
	...
	if (rml_c_heap->region == rml_c_heap->next) return;
    ...
    void **next, char *region_low, rml_uint_t region_nbytes) 
{
  for (; nwords > 0; ++scan, --nwords) 
  {
    ...
    {
		/* If is not allocated in the C heap, no not forward it! */
		if (rml_c_heap_collect_flag && rml_is_allocated_on_c_heap(old))
		{
    ...
    char *region_low, rml_uint_t region_nbytes) 
{
   ...
   */
  {
    ...
    next = rml_forward_vec(ATP, (rml_uint_t)cnt, next, region_low,
        region_nbytes);
    ...
    for (; --cnt >= 0; ++ATP) 
    {
      ...
      if ( RML_HDRISFORWARD(nrelements)) { continue; }
...
static void **rml_collect(void **scan, char *region_low,
    rml_uint_t region_nbytes, rml_uint_t nliveargs) 
{
  void **next;
...
static void rml_major_collection(rml_uint_t nwords, rml_uint_t nliveargs) 
{
  ...
  {
    if (rml_c_heap_region_total_size + nwords < rml_older_size
        - (rml_current_next - rml_current_region))
  ...
  if ( 10 * current_inuse > 9*rml_older_size) /* current_inuse > 90/100 * rml_older_size */
  {
    ...
    new_size = (current_inuse + current_inuse/4) + rml_young_size;
  ...
  if (rml_flag_gclog && !rml_flag_bench)
    fprintf(stderr, " %lu%% used]\n", (unsigned long)((current_inuse*100)
        /rml_older_size));
}
...
void rml_minor_collection(rml_uint_t nliveargs) 
{
  ...
  next = rml_collect(rml_current_next, (char*)rml_young_region, rml_young_size
      /* RML_YOUNG_SIZE */* sizeof(void*), nliveargs);
