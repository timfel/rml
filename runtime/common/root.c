...
rml_sint_t    rml_global_roots_trail_index_max = -1;

/* forward my external roots */
void rml_roots_gc(struct rml_xgcstate *state)
{
  if (rml_global_roots_trail_index_max >= 0) /* if not negative */
    rml_roots_gc_callback(
      state, 
      rml_global_roots_trail, 
      rml_global_roots_trail_index_max*sizeof(void*));
}
...
RML_BEGIN_LABEL(RML__get_5fglobal_5froot)
{
  ...
  if (i > rml_global_roots_trail_index_max || i >= rml_global_roots_trail_size)
    RML_TAILCALLK(rmlFC);
