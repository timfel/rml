...
functor FOLUnifyFn(structure Util : UTIL
  ...
  struct
    ...
    fun e_var(var1, var2) =
      let val FOL.VAR{inst=r1,...} = FOL.deref var1
	  and FOL.VAR{inst=r2,...} = FOL.deref var2
      in
    ...
    fun u_var(var1, var2) =
      let val FOL.VAR{inst=r1,...} = FOL.deref var1
	  and (var2 as FOL.VAR{inst=r2,...}) = FOL.deref var2
      in
		if r1 = r2 then ()
	...
	(* adrpo added first 2 lines from this function to unify RML.debug calls
	   ...
	   *)
    fun u_conj(FOL.CALL(vref1 as FOL.GVAR(FOL.LONGID(SOME("RML"), debug1)), 
						expl1, 
						varl1), 
			   FOL.CALL(vref2 as FOL.GVAR(FOL.LONGID(SOME("RML"), debug2)), 
						expl2, 
						varl2)) =
	   (* before we compare expressions lists we have to take into consideration
	      the "not" goals, if they exists *)
	   if ((String.isPrefix "debug" debug1) andalso 
	       (String.isPrefix "debug" debug2) andalso 
	       (debug1 = debug2))
	   then 
	     (if (debug1 = "debug")
	      then 
      ...
      |u_conj(FOL.CALL(vref1, expl1, varl1), FOL.CALL(vref2, expl2, varl2)) =
	  (e_vref(vref1,vref2); zipapp(u_exp,expl1,expl2); zipapp(u_var,varl1,varl2))
      | u_conj(FOL.MATCH mrules1, FOL.MATCH mrules2) = u_mrules(mrules1, mrules2)
      | u_conj(FOL.EQUAL(var1, exp1), FOL.EQUAL(var2, exp2)) =
	  (e_var(var1, var2); u_exp(exp1, exp2))
      | u_conj(FOL.BIND(var1, exp1), FOL.BIND(var2, exp2)) =
	  (u_var(var1, var2); u_exp(exp1, exp2))
      | u_conj(FOL.NOT conj1, FOL.NOT conj2) = u_conj(conj1, conj2)
      | u_conj(FOL.AND(c11, c12), FOL.AND(c21, c22)) =
