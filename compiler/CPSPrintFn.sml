...
functor CPSPrintFn(structure MakeString : MAKESTRING
  ...
  struct
    ...
    fun unary(CPS.FETCH off)        = "fetch "^(MakeString.icvt off)
      ...
      | unary(CPS.INT_BIT_NOT)      = "int_bit_not"
      | unary(CPS.REF_INT)          = "reference_integer"
      | unary(CPS.VAL_CONSTR)       = "value_constructor"
      | unary(CPS.VAL_SLOTS)        = "value_slots"
      | unary(CPS.STRING_HASH)      = "string_hash"
      | unary(CPS.STRING_HASH_DJB2) = "string_hash_djb2"
      | unary(CPS.STRING_HASH_SDBM) = "string_hash_sdbm"
      

    fun binary(CPS.EQUAL)          = "equal"
      | binary(CPS.BOOL_AND)       = "bool_and"
      | binary(CPS.BOOL_OR)        = "bool_or"
      | binary(CPS.BOOL_EQ)        = "bool_eq"
      | binary(CPS.CHAR_EQ)        = "char_eq"
      ...
      | binary(CPS.INT_BIT_AND)    = "int_bit_and"
      | binary(CPS.INT_BIT_OR)     = "int_bit_or"
      | binary(CPS.INT_BIT_XOR)    = "int_bit_xor"
      | binary(CPS.INT_BIT_LSHIFT) = "int_bit_lshift"
      | binary(CPS.INT_BIT_RSHIFT) = "int_bit_rshift"
      | binary(CPS.REAL_EQ)        = "real_eq"
      | binary(CPS.STRING_EQ)      = "string_eq"
      | binary(CPS.REF_EQ)         = "reference_eq"
      | binary(CPS.VAL_EQ)         = "value_eq"
