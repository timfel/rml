...
functor CodeToCFn(structure MakeString : MAKESTRING
  ...
  fun prLVarName os lvar = output(os, Code.lvarStringName lvar)
  ...
  fun  instrumentLabel os label =
    ...
    and prCode' os (Code.GOTO(target, nargs, name, pos, gototy)) =
      ...
      | prCode' os (Code.SWITCH(_, [], NONE, _)) = bug "prCode': SWITCH(_,[],NONE)"
      | prCode' os (Code.SWITCH(v, [], SOME default, _)) = (* == BIND(NONE,v,default) *)
        (output(os, "\n  "); prVal os v; output(os, ";"); prCode os default)
      | prCode' os (Code.SWITCH(v, (case0 as (Code.INTct _,_))::cases, default, _)) =
        if List.length cases < !Control.switchRewriteThreshold then
          ...
          (output(os, "\n  switch( RML_UNTAGFIXNUM(");
           ...
           output(os, "\n  }"))
      | prCode' os (Code.SWITCH(v, (case0 as (Code.HDRct _,_))::cases, default, _)) =
        if List.length cases < !Control.switchRewriteThreshold then
          (output(os, "\n  switch( (rml_sint_t)");
           prVal os v;
           output(os, " ) {");
      ...
      | prCode' os (Code.SWITCH(v, cases as ((Code.STRINGct _,_)::_), default, xvar)) =
        let 
        in
          output(os, "\n  "); prLVar os xvar; output(os, " = "); prVal os v; output(os, ";\n  ");
          List.app (prStringCase os xvar) cases; 
          output(os, "{");
          prStringDefault os default;
          output(os, "}")
        end
    ...
    fun prValDec os lab = (output(os, "extern struct rml_gval "); prLabel os lab; output(os, ";\n"))

    fun prValDef os (lab,lr) =
      let fun prLitRefConst(_, Code.EXTERNlr _) = ()
          |   prLitRefConst(os, _) = output(os, "const ")
          fun prLitRefDefn(os, Code.EXTERNlr extlab) = (output(os, "&"); prLabel os extlab)
    ...
    fun emitValues _ [] = ()
      | emitValues os values = (output(os, "\n"); List.app (prValDef os) values)

    fun prInitVal os (lab,lr) =
      case lr of Code.EXTERNlr _ => (output(os, "  rml_gval_init(&"); prLabel os lab; output(os, ");\n"))
      | _ => ()
