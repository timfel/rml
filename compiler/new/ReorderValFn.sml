...
functor ReorderValFn(structure Util : UTIL
  ...
  struct
	...
	of Absyn.WILDpat (_) => VE
	 | Absyn.LITpat (_,_) => VE
	 | Absyn.CONpat (_,_) => VE
	 | Absyn.STRUCTpat(_,pats,_) => List.foldl scanPat VE pats
    ...
    fun depsOfGoal(goal, (VE,deps)) =
      case goal
	of Absyn.CALLgoal(longid, exps, pats, _) =>
    ...
    fun depsOfClause(VE, clause, deps) =
      case clause
	of Absyn.CLAUSE1(goal_opt, _, pats, result, _) =>
    ...
    fun analyseValBnd source VE depArr (i,VALBND(var,exp)) =
      ...
      | analyseValBnd source VE depArr (i,RELBND(Absyn.RELBIND(_,_,clause, _))) =
			Array.update(depArr, i, depsOfClause(VE,clause,[]))

    (* REORDERING *)

    fun varOf(VALBND(var,_)) = var
      | varOf(RELBND(Absyn.RELBIND(var, _, _, _))) = var
    ...
    fun mkValDec source valbLst =
      let fun split([], [], []) = bug "mkValDec: split([],[],[])"
		...
		let fun sayRel(Absyn.RELBIND(var, _, _, _)) =
