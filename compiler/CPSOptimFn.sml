...
functor CPSOptimFn(structure Util : UTIL
  ...
  struct
    ...
    fun not_free_in vars =
	  ...
	  and notin_exp'(CPS.AppFCe(t_fc)) = notin_te t_fc
    ...
    fun find_matching_case(lit, cases, default) =
	  ...
	  fun loop([]) =
		...
		if found const then exp	(* beta_sw1 *)
		else loop cases
      in
	loop cases
      end
     ...
     *)
    fun fetch_lit(0, CPS.STRUCTlit(con,lits)) =
	  SOME(CPS.CONSTlit(CPS.HDRcon{con=con, len=length lits}))
      | fetch_lit(off, CPS.STRUCTlit(_,lits)) = SOME(List.nth(lits, off-1))
	...
	of CPS.VARte _			=> ()
	 ...
	 | CPS.LAMte{kind,body,...}	=>
		...
		of CPS.FClk		=>
		    (case exp
		       of CPS.AppFCe(t_fc) => root <-- CPS.getTE t_fc	(* eta_fc *)
			| _		=> ())
		 | CPS.SClk{v_tvs}	=>
		    (case exp
		       of CPS.AppSCe{sc,args}	=>
			    if not_free_in v_tvs sc andalso
			       same_vars_ts(v_tvs, args)
	...
	of CPS.MARKERp		=> reduce_exp e1
	 | CPS.MKSTRUCTp(con,ts) =>
		...
		let val lit = CPS.STRUCTlit(con, map get_quote ts)
		in
	...
	of CPS.AppFCe(t_fc)	=>
		...
		of CPS.QUOTEte(CPS.PROClit proc)	=>
		    (case proc
		       of CPS.LOCAL_REL(CPS.DEF{uses as ref 1,v_tvs,v_fc,v_sc,body,...}) =>
			    let val e = CPS.LETe(v_fc, fc,
						 CPS.EXP(ref(CPS.LETe(v_sc, sc, body))))
			    in
	 ...
	 | CPS.AppLABe(CPS.LAB{uses,bvars,body,...}, t_star) =>
	    if !uses = 1 then
	      (root0 <-- inline_app(bvars, t_star, CPS.getExp body);
	       reduce_exp e0)
